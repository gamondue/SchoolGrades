...
namespace SchoolGrades.BusinessObjects
{
    public class Topic
    {
        ...
        // 2025-11-12 CORREZIONE: getter e setter erano invertiti
        public int? ParentNodeOld { get => parentNodeOld; set => parentNodeOld = value; }
        public int? ParentNodeNew { get => parentNodeNew; set => parentNodeNew = value; }
        ...
        // Metodo per creare una copia profonda dell'oggetto Topic
        public Topic Clone()
        {
            return new Topic
            {
                id = this.id,
                parentNodeNew = this.parentNodeNew,
                parentNodeOld = this.parentNodeOld,
                leftNodeOld = this.leftNodeOld,
                leftNodeNew = this.leftNodeNew,
                rightNodeOld = this.rightNodeOld,
                rightNodeNew = this.rightNodeNew,
                childNumberNew = this.childNumberNew,
                childNumberOld = this.childNumberOld,
                name = this.name,
                desc = this.desc,
